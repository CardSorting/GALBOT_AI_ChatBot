...
class ImagineCommandHandler {
    ...
    async handleInteraction(interaction, prompt) {
...
async imageWorker(task, callback) {
    try {
        ...
        await task.interaction.editReply(backblazeUrl); // Use Backblaze URL here
    } catch (error) {
        logger.error({
        ...
        await task.interaction.editReply('Error in processing your request.');
    } finally {
        callback();}
    }
}
